...
module.exports = (grunt) ->
  ...
  grunt.initConfig
    ...
    coffee:
      compile:
        files:
          'backbone.list.js': [ 'src/backbone.list.coffee' ]
    ...
    uglify:
      main:
        files:
          'backbone.list.min.js': [ 'backbone.list.js' ]

    karma:
      options:
        browsers: [ 'PhantomJS' ]
        frameworks: [ 'mocha', 'chai' ]
        files: [ 'backbone.list.js', 'test/**/*.coffee' ]
    ...
    replace:
      ...
      debugJS:
        src: [ 'backbone.list.js' ]
        overwrite: true
        replacements: [
          from: /^/
          to: -> "/* backbone.list.js v#{ packageVersion() }" +
            " (coffeescript output) */ \n\n"
        ]

      minJS:
        src: [ 'backbone.list.min.js' ]
        overwrite: true
        replacements: [
          from: /^/
          to: -> "/* backbone.list.min.js v#{ packageVersion() } */ \n"
        ]
    ...
    bump:
      options:
        ...
        commitFiles: [
          'package.json'
          'src/backbone.list.coffee'
          'backbone.list.js'
          'backbone.list.min.js'
        ]
        pushTo: 'origin'
